...
-- Drop existing overly permissive RLS policy on contact_requests
DROP POLICY IF EXISTS "Only authenticated users can view contact requests" ON public.contact_requests;

-- Create new restrictive RLS policy for contact_requests
...
-- (the existing "Anyone can submit contact requests" policy remains)
